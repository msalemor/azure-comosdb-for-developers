...
namespace Microsoft.Extensions.Caching.Cosmos
{
    ...
    public class CosmosCache : IDistributedCache, IDisposable
    {
        ...
        public static double LastWriteCharge;
        public static string LastWriteRegion;
        public static string LastWriteOperation;
        public static DateTime? LastWriteOperationTS;
        public static double LastReadCharge;
        public static string LastReadRegion;
        public static string LastReadOperation;
        public static DateTime? LastReadOperationTS;
        public static double LastDeleteCharge;
        public static string LastDeleteRegion;
        public static string LastDeleteOperation;
        public static DateTime? LastDeleteOperationTS;
        ...
        private void SetReadInformation(double charge, string region, string operation)
        {
            lock (obj)
            {
                LastReadCharge = charge;
                LastReadRegion = region;
                LastReadOperation = operation;
                LastReadOperationTS = DateTime.UtcNow;
            }
        }

        private void SetWriteInformation(double charge, string region, string operation)
        {
            lock (obj)
            {
                LastWriteCharge = charge;
                LastWriteRegion = region;
                LastWriteOperation = operation;
                LastWriteOperationTS = DateTime.UtcNow;
            }
        }

        private void SetDeleteInformation(double charge, string region, string operation)
        {
            lock (obj)
            {
                LastDeleteCharge = charge;
                LastDeleteRegion = region;
                LastDeleteOperation = operation;
                LastDeleteOperationTS = DateTime.UtcNow;
            }
        }
        ...
        public async Task<byte[]> GetAsync(string key, CancellationToken token = default(CancellationToken))
        {
            ...
            try
            {
                ...
                SetReadInformation(cosmosCacheSessionResponse.RequestCharge, this.cosmosClient.Endpoint.ToString(), $"Read key: {key}");
            ...
            try
            {
                var operation = await this.cosmosContainer.ReplaceItemAsync(
                        partitionKey: new PartitionKey(key),
                        id: key,
                ...
                SetWriteInformation(operation.RequestCharge, this.cosmosClient.Endpoint.ToString(), $"Replace Item: {key}");
        ...
        public async Task RemoveAsync(string key, CancellationToken token = default(CancellationToken))
        {
            ...
            var operation = await this.cosmosContainer.DeleteItemAsync<CosmosCacheSession>(
                partitionKey: new PartitionKey(key),
                id: key,
                requestOptions: null,
                cancellationToken: token).ConfigureAwait(false);

            SetDeleteInformation(operation.RequestCharge, this.cosmosClient.Endpoint.ToString(), $"Deleted key: {key}");
        }
